/**
 * Reusable gradient+3D button OG image layout for locale pages.
 * Same style as sound detail opengraph-image (yellow→pink→blue gradient, decorative circles).
 */

import { OG_IMAGE_WIDTH, OG_IMAGE_HEIGHT } from "./StaticOgImage"
...
function SoundButtonSvg({ entityId, size: sz }: { entityId: number; size: number }) {
  const innerHex = getButtonColor(entityId)
  const rgb = hexToRgb(innerHex)
  const bid = `locale-og-${entityId}`
...
export const LOCALE_OG_SIZE = { width: OG_IMAGE_WIDTH, height: OG_IMAGE_HEIGHT }

export interface LocaleOgImageProps {
  /** Main title to display (e.g. "Botones de Sonido", sound name, category name) */
  title: string
  /** Entity id for button color (sound id, category id, etc.) - defaults to 1 */
  entityId?: number
}

export function LocaleOgImage({ title, entityId = 1 }: LocaleOgImageProps) {
  const displayName = truncate(title, 40)
  return (
    <div
      style={{
        width: "100%",
    ...
    >
      ...
      >
        <SoundButtonSvg entityId={entityId} size={360} />
        <div
          style={{
            marginTop: 20,
            fontSize: 56,
