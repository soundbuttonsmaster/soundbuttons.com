...
import { getPublicDashboards, slugifyDashboardName } from "@/lib/api/dashboards"
import { OG_IMAGE_WIDTH, OG_IMAGE_HEIGHT } from "@/components/og/StaticOgImage"

export const alt = "Soundboard - SoundButtons.com"
...
function SoundButtonSvg({ id, size: sz }: { id: number; size: number }) {
  const innerHex = getButtonColor(id)
  const rgb = hexToRgb(innerHex)
  const bid = `sb-dash-og-${id}`
...
export default async function OgImage({
  ...
  const displayName = truncate(dashboard?.name ?? "Soundboard", 40)
  const dashboardId = dashboard?.id ?? 1
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        <SoundButtonSvg id={dashboardId} size={360} />
        <div
          style={{
            marginTop: 20,
            fontSize: 56,
