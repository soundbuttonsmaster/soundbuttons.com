...
function getButtonColor(soundId: number): string {
  return COLOR_PALETTE[soundId % COLOR_PALETTE.length] ?? "#3498DB"
}
...
/** 3D sound button SVG - same as sound-button component */
function SoundButtonSvg({
  soundId,
  size: sz,
}: {
  soundId: number
  ...
  const innerHex = getButtonColor(soundId)
  const rgb = hexToRgb(innerHex)
  const bid = `og-${soundId}`
...
export default async function OgImage({
  ...
  const displayName = truncate(soundName, 40)
  ...
  return new ImageResponse(
    ...
    >
      ...
      {/* Center: 3D sound button (same as sound-button component) + title */}
      <div
        style={{
          display: "flex",
          flexDirection: "column",
      ...
      >
        <SoundButtonSvg soundId={soundId} size={360} />
        <div
          style={{
            marginTop: 20,
            fontSize: 56,
