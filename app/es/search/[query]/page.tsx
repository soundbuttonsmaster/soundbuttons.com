...
export async function generateMetadata({
  ...
  if (!searchQuery || searchQuery.length < 2) return { title: "Buscar - Sound Buttons" }
...
export default async function EsSearchResultsPage({
  params,
}: {
  params: Promise<{ query: string }>
}) {
  ...
  if (!searchQuery || searchQuery.length < 2) redirect("/es/search")

  const sanitizedQuery = searchQuery.replace(/[<>]/g, "").trim()
  if (!sanitizedQuery) redirect("/es/search")
