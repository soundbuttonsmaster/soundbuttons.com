...
export default function MySoundboardClient() {
  ...
  const loadSounds = useCallback(
    async (pageNum = 1, append = false) => {
      if (!token) return
      const result = await apiClient.getMySounds(token, pageNum, PAGE_SIZE)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    loadSounds(1, false).finally(() => setLoading(false))
  }, [isReady, token, loadSounds])
  ...
  const handleLoadMore = async (): Promise<boolean> => {
    ...
    try {
      await loadSounds(nextPage, true)
      setPage(nextPage)
      return hasMore
    } finally {
          ...
          <div className="max-w-md mx-auto px-4 text-center">
            <div className="bg-card border border-border rounded-2xl p-8 space-y-6">
              ...
              <p className="text-muted-foreground text-sm">
                Your uploaded sounds and custom soundboard will appear here.
              </p>
              <Button asChild className="w-full h-11">
                <Link href="/login?redirect=/my-soundboard">
  ...
  return (
    <>
      <PageHero
        title="My Soundboard"
        description="Your personal soundboard. Play and manage your sounds."
      />
      <div className="py-8 bg-background">
        <div className="w-full max-w-7xl mx-auto px-4">
          {loading ? (
            <div className="min-h-[200px] flex items-center justify-center text-muted-foreground">
              Loading your soundboard...
            </div>
          ) : sounds.length === 0 ? (
            <div className="text-center py-12">
              <LayoutGrid className="mx-auto h-12 w-12 text-muted-foreground/50 mb-4" />
              <h2 className="text-lg font-semibold text-foreground mb-2">Your soundboard is empty</h2>
              <p className="text-muted-foreground text-sm mb-4">
                Upload sounds or add favorites to build your soundboard.
              </p>
              <div className="flex flex-wrap justify-center gap-3">
                <Button asChild variant="outline">
                  <Link href="/upload-sound">Upload sound</Link>
                </Button>
                <Button asChild variant="outline">
