...
export const alt = "Blog Post - SoundButtons.com"
...
/** 3D sound button SVG - same as sound detail / categories */
function SoundButtonSvg({ blogId, size: sz }: { blogId: number; size: number }) {
  const innerHex = getButtonColor(blogId)
  const rgb = hexToRgb(innerHex)
  const bid = `blog-og-${blogId}`
...
export default async function OgImage({
  ...
  const displayName = truncate(blogTitle, 40)
  ...
  return new ImageResponse(
    ...
    >
      {/* Decorative circles - play-random / sound detail style */}
      <div
        style={{
          position: "absolute",
          top: 60,
      ...
      {/* Center: 3D sound button + blog title */}
      <div
        style={{
          display: "flex",
          flexDirection: "column",
      ...
      >
        <SoundButtonSvg blogId={blogId} size={360} />
        <div
          style={{
            marginTop: 20,
            fontSize: 56,
